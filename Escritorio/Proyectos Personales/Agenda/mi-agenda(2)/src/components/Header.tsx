      ...
      <div className="max-w-[1600px] 2xl:max-w-[1780px] mx-auto px-3 sm:px-6 lg:px-8 py-2 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-3 flex-nowrap py-0.5">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            {/* Import / Export / Backup Box & Cloud Sync */}
            <div className="flex items-center gap-1.5 shrink-0">
              {onManualSync && (
                <button
                  ...
                  title="Sincronizar turnos, pacientes y datos en tiempo real entre PC y Celular"
                >
                  ...
                  <span className="sm:hidden">{isSyncingCloud ? '...' : 'Sincro'}</span>
                </button>
              )}
              ...
              <div className="flex items-center bg-black/25 rounded-xl border border-white/20 p-0.5 sm:p-1 shrink-0">
                {onExportNextDayAppointmentsTxt && (
                  <button
                    onClick={onExportNextDayAppointmentsTxt}
                    title="Descargar archivo de texto (.txt) con los turnos del día siguiente para contingencias"
                    className="flex items-center gap-1 px-2 py-1 text-white/90 hover:text-white transition-colors rounded-lg hover:bg-white/10 text-[11px] font-bold cursor-pointer"
                  >
                    <FileText className="w-3.5 h-3.5 text-amber-300" />
                    <span className="hidden xl:inline">Turnos Mañana (.txt)</span>
                  </button>
                )}
                {onExportContactsTxt && (
                  <button
                    onClick={onExportContactsTxt}
                    title="Descargar listado completo de pacientes registrados en archivo de texto plano (.txt)"
                    className="flex items-center gap-1 px-2 py-1 text-white/90 hover:text-white transition-colors rounded-lg hover:bg-white/10 text-[11px] font-bold cursor-pointer"
                  >
                    <Users className="w-3.5 h-3.5 text-emerald-300" />
                    <span className="hidden xl:inline">Pacientes (.txt)</span>
                  </button>
                )}
                <button
                  onClick={onExportJson}
                  title="Exportar copia de seguridad completa (.json)"
                ...
                <button
                  ...
                  title="Importar contactos o restaurar copia"
                  className="p-1.5 text-white/80 hover:text-white transition-colors rounded-lg hover:bg-white/10 cursor-pointer"
                >
