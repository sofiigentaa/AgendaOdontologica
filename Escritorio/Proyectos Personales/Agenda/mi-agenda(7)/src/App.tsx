...
import { PatientConfirmationView } from './components/PatientConfirmationView';
...
export default function App() {
  ...
  const mergeAppointmentsWithState = (incomingAppts: Appointment[], prevAppts: Appointment[]): Appointment[] => {
    ...
    prevAppts.forEach((a) => currentMap.set(a.id, a));
  ...
  const updateReminders = (newReminders: CallReminder[]) => {
    ...
    syncToSupabase({ reminders: newReminders });
  };
  ...
  const handleUpdateAppointmentStatus = (
    ...
    if (status === 'confirmed') {
      ...
      showToast('❌ Turno marcado como Cancelado (horario libre).');
    } else {
      showToast('⏳ Estado del turno actualizado a pendiente.');
    }
  };
  ...
  const updateInsuranceFiles = (newFiles: InsuranceFolderFile[]) => {
    ...
    syncToSupabase({ insuranceFiles: newFiles });
  };
  ...
  const handleExportContactsTxt = () => {
    exportContactsPlainFile(contacts);
    showToast('📄 Archivo plano de pacientes descargado');
  };

  const handleExportNextDayAppointmentsTxt = () => {
    exportNextDayAppointmentsPlainFile(appointments, contacts);
    showToast('📅 Archivo plano con turnos del día siguiente descargado');
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!rawContent || !rawContent.trim()) {
          showToast('⚠️ El archivo seleccionado está vacío');
          return;
        }
        ...
        try {
          ...
          showToast('❌ El archivo no tiene formato JSON válido');
          return;
        }

        let importedContactsCount = 0;
        let importedApptsCount = 0;

        // Case 1: Direct array of contacts or appointments
        if (Array.isArray(parsed)) {
          // Check if it's contacts or appointments
          if (parsed.length > 0 && ('date' in parsed[0] || 'time' in parsed[0] || 'motive' in parsed[0])) {
            updateAppointments(parsed);
            importedApptsCount = parsed.length;
          } else {
            // Assume contacts
            const validContacts = parsed.map((c: any, idx: number) => ({
              id: c.id || `contact-${Date.now()}-${idx}`,
              fullName: c.fullName || c.name || c.full_name || 'Sin Nombre',
              primaryPhone: c.primaryPhone || c.phone || c.primary_phone || '',
              secondaryPhone: c.secondaryPhone || c.secondary_phone || '',
              email: c.email || '',
              insuranceName: c.insuranceName || c.insurance_name || (c.isParticular ? 'Particular' : 'Particular'),
              insuranceNumber: c.insuranceNumber || c.insurance_number || '',
              isParticular: c.isParticular ?? c.is_particular ?? true,
              dni: c.dni || '',
              ...
              avatarColor: c.avatarColor || c.avatar_color || 'bg-emerald-100 text-emerald-700',
            ...
            importedContactsCount = validContacts.length;
          }
        } 
        // Case 2: Full Backup object structure
        else if (typeof parsed === 'object' && parsed !== null) {
          if (Array.isArray(parsed.contacts)) {
            updateContacts(parsed.contacts);
            importedContactsCount = parsed.contacts.length;
          }
          if (Array.isArray(parsed.appointments)) {
            updateAppointments(parsed.appointments);
            importedApptsCount = parsed.appointments.length;
          }
          if (Array.isArray(parsed.reminders)) {
            updateReminders(parsed.reminders);
          }
          if (Array.isArray(parsed.notes)) {
            updateNotes(parsed.notes);
          }
          if (Array.isArray(parsed.attachments)) {
            updateAttachments(parsed.attachments);
          }
          if (Array.isArray(parsed.insuranceFiles)) {
            setInsuranceFiles(parsed.insuranceFiles);
            saveStoredInsuranceFiles(parsed.insuranceFiles);
          }
        }

        const summary = [];
        if (importedContactsCount > 0) summary.push(`${importedContactsCount} pacientes`);
        if (importedApptsCount > 0) summary.push(`${importedApptsCount} turnos`);

        if (summary.length > 0) {
          showToast(`✅ Copia de seguridad importada con éxito: ${summary.join(', ')}`);
        } else {
          showToast('✅ Archivo JSON importado correctamente');
        }
      } catch (err: any) {
        console.error('Error importing JSON:', err);
        showToast(`❌ Error al importar: ${err?.message || 'Formato de archivo no compatible'}`);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] text-[#333333] font-sans flex flex-col w-full max-w-full overflow-x-clip">
      ...
      <Header
        ...
        onExportContactsTxt={handleExportContactsTxt}
        onExportNextDayAppointmentsTxt={handleExportNextDayAppointmentsTxt}
        onLogout={() => {
