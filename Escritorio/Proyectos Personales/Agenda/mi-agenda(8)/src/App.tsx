  ...
  const handleImportJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const parsedData = parseImportFileContent(rawContent, file.name);
        ...
        if (parsedData.notes && parsedData.notes.length > 0) {
          updateNotes(parsedData.notes);
        }
        if (parsedData.attachments && parsedData.attachments.length > 0) {
          updateAttachments(parsedData.attachments);
        }
        if (parsedData.insuranceFiles && parsedData.insuranceFiles.length > 0) {
          setInsuranceFiles(parsedData.insuranceFiles);
          saveStoredInsuranceFiles(parsedData.insuranceFiles);
        }

        showToast(parsedData.summaryMessage || '✅ Archivo importado correctamente');
      } catch (err: any) {
