...
export const AlertsScreen: React.FC<AlertsScreenProps> = ({ onGoToInventory, onGoToPOS }) => {
  ...
  const handleApplyDiscount = (p: Product, discountPct: number) => {
    const discountedPrice = Math.round((p.salePrice * (1 - discountPct / 100)) / 10) * 10;
    updateProduct(p.id, { salePrice: discountedPrice });
    setSuccessToast('Precio de liquidación aplicado: ' + formatCurrency(discountedPrice) + ' (-' + discountPct + '%)');
    setTimeout(() => setSuccessToast(null), 3000);
  };
      ...
      <div className="space-y-3.5">
                ...
                <div key={p.id} className="py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span>Ingresar +{p.minStock * 2} u.</span>
                    </button>
                  </div>
                </div>
              ))}
        ...
        {(activeSubTab === 'ALL' || activeSubTab === 'EXPIRY') && (expiredProducts.length > 0 || expiringProducts.length > 0) && (
          <div className="bg-white rounded-lg border border-amber-200 shadow-xs overflow-hidden">
            <div className="bg-amber-600 px-4 py-2.5 text-white flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-amber-700 px-2 py-0.5 rounded font-bold">
                    ...
                    <div>
                      <div className="font-bold text-slate-900 text-xs flex items-center gap-2">
                        ...
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-900 border border-amber-200 font-mono">
                          Vence en {days} días ({formatDate(p.expirationDate!)})
                        </span>
                      </div>
                      <div className="text-[11px] text-slate-500 mt-0.5 font-mono">
                        Stock: {p.stock} u. • Precio: {formatCurrency(p.salePrice)}
                      </div>
                    </div>

                    <div className="flex flex-wrap items-center gap-1.5">
                      <button
                        onClick={() => handleApplyDiscount(p, 20)}
                        className="px-2.5 py-1 rounded bg-amber-50 hover:bg-amber-100 text-amber-800 text-xs font-bold border border-amber-300 font-mono"
                        title="Aplicar 20% de descuento para venta rápida antes de vencer"
                      >
                        Liquidación -20% ({formatCurrency(p.salePrice * 0.8)})
                      </button>
                    </div>
                  </div>
                );
          ...
          <div className="bg-white w-full max-w-sm rounded-lg p-5 shadow-xl border border-slate-200 space-y-3.5">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-1.5 rounded bg-indigo-600 text-white text-xs font-bold shadow-xs hover:bg-indigo-700"
              >
                Confirmar Ingreso
              </button>
            </div>
