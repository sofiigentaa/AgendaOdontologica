...
export const ReportsScreen: React.FC<ReportsScreenProps> = ({ onGoToInventory }) => {
  ...
  const [topTab, setTopTab] = useState<'UNITS' | 'REVENUE' | 'PROFIT'>('UNITS');
  ...
  const topProducts = useMemo(() => {
    ...
    if (topTab === 'UNITS') list.sort((a, b) => b.unitsSold - a.unitsSold);
    else if (topTab === 'REVENUE') list.sort((a, b) => b.revenue - a.revenue);
    else list.sort((a, b) => b.profit - a.profit);

    return list.slice(0, 10);
  }, [filteredSales, products, topTab]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 space-y-3.5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-lg border border-slate-200 shadow-xs">
        <div>
          <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>Reportes, Métricas & Estadísticas de Ventas</span>
          </h2>
          <p className="text-[11px] text-slate-500 mt-0.5">
            Analice márgenes de ganancia, productos más vendidos y tendencias de consumo para compras estratégicas.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition-all border border-slate-200 flex items-center gap-1"
            title="Imprimir informe y gráficos de estadísticas"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-xs">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Markup / Recargo</div>
          <div className="text-xl font-bold font-mono text-indigo-600 mt-0.5">{formatPercent(markupPercent)}</div>
          <div className="text-[11px] text-slate-500 mt-0.5">Sobre costo de mercadería</div>
        </div>

        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-xs">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Ticket Promedio</div>
          <div className="text-xl font-bold font-mono text-slate-800 mt-0.5">{formatCurrency(averageTicket)}</div>
          <div className="text-[11px] text-slate-500 mt-0.5">Gasto medio por cliente</div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <div className="flex gap-1 bg-slate-100 p-0.5 rounded border border-slate-200">
              {[
                ...
                <button
                  key={tb.id}
                  onClick={() => setTopTab(tb.id as any)}
                  ...
                  <div className="flex items-center gap-3 shrink-0 text-right">
                    <div>
                      <div className="font-bold font-mono text-slate-900 text-xs">
                        {topTab === 'UNITS' && p.unitsSold + ' u.'}
                        {topTab === 'REVENUE' && formatCurrency(p.revenue)}
                        {topTab === 'PROFIT' && formatCurrency(p.profit)}
                      </div>
                      <div className="text-[10px] text-slate-400 font-mono">
                        {p.unitsSold} vendidas • {formatCurrency(p.profit)} ganancia
                      </div>
                    </div>
