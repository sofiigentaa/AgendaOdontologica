...
interface HeaderProps {
  ...
  onManualSync?: () => void;
  onExportContactsTxt?: () => void;
  onExportNextDayAppointmentsTxt?: () => void;
  onLogout?: () => void;
}
...
export const Header: React.FC<HeaderProps> = ({
  ...
  onManualSync,
  onExportContactsTxt,
  onExportNextDayAppointmentsTxt,
  onLogout,
}) => {
  ...
  // Close backup menu on click outside
  useEffect(() => {
  ...
  return (
    <header 
      ref={headerRef} 
      className="bg-[#2E7D5E] text-white border-b border-[#24664c] sticky top-0 left-0 right-0 w-full z-40 shadow-md max-w-full"
    >
      <div className="max-w-[1600px] 2xl:max-w-[1780px] mx-auto px-3 sm:px-6 lg:px-8 py-2.5 space-y-2.5">
        
        {/* Main Row: Brand Logo, Navigation Tabs & Right-side Action Tools */}
        <div className="flex items-center justify-between gap-3 flex-wrap sm:flex-nowrap">
          
          {/* Title & Brand Isotype */}
          <div className="flex items-center gap-2.5 shrink-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-white/15 border border-white/25 flex items-center justify-center text-white shadow-xs shrink-0 p-1.5 backdrop-blur-xs">
            ...
            <div className="flex flex-col justify-center">
              <h1 className="text-sm sm:text-base font-bold tracking-tight text-white leading-tight notranslate" translate="no">
                Consultorio Marie - Yani
              </h1>
              <p className="text-[10.5px] text-emerald-100/90 font-medium tracking-wide leading-tight">
                Agenda odontológica
              </p>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border shrink-0 whitespace-nowrap cursor-pointer active:scale-95 min-h-[34px] ${
                activeTab === 'contacts'
                  ? 'bg-white text-[#2E7D5E] border-white shadow-2xs font-extrabold'
                  : 'bg-black/20 text-white/90 border-white/20 hover:text-white hover:bg-white/10'
              }`}
            >
          ...
          {/* Right Tools (Google Account, Cloud Sync & Clean Export Dropdown) */}
          <div className="flex items-center gap-2 shrink-0 ml-auto sm:ml-0">
            ...
            {onManualSync && (
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold transition-all border cursor-pointer active:scale-95 shadow-xs shrink-0 ${
                  isSyncingCloud
                    ? 'bg-amber-400 text-slate-900 border-amber-300 animate-pulse'
                    : 'bg-white/15 hover:bg-white/25 active:bg-white/30 text-white border-white/30'
                }`}
                title="Sincronizar turnos y pacientes en la nube"
              >
                <RotateCcw className={`w-3.5 h-3.5 ${isSyncingCloud ? 'animate-spin' : ''}`} />
                <span className="hidden lg:inline">{isSyncingCloud ? 'Sincronizando...' : 'Sincronizar'}</span>
              </button>
            )}
            ...
            {googleUser ? (
              ...
              >
                ...
                <div className="hidden xl:flex flex-col text-left leading-none">
                  <span className="text-[11px] font-bold text-slate-900 truncate max-w-[110px]">{googleUser.displayName || googleUser.email?.split('@')[0]}</span>
                  <span className="text-[9px] text-emerald-700 font-bold">Google ✔</span>
                </div>
                {onDisconnectGoogle && (
                  <button
            ...
            <div className="relative shrink-0" ref={backupMenuRef}>
              <button
                ...
                className="flex items-center gap-1.5 bg-black/25 hover:bg-black/35 text-white px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold border border-white/20 transition-all cursor-pointer shadow-xs active:scale-95"
                title="Menú de exportación, copias de seguridad y contingencias"
              >
                <Download className="w-3.5 h-3.5 text-emerald-200" />
                <span className="hidden sm:inline">Exportar / Copias</span>
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isBackupMenuOpen ? 'rotate-180' : ''}`} />
              </button>

              {/* Floating Dropdown Panel */}
              {isBackupMenuOpen && (
                <div className="absolute right-0 mt-1.5 w-72 sm:w-80 bg-white text-slate-800 rounded-2xl shadow-2xl border border-slate-200 p-2 z-50 animate-in fade-in zoom-in-95">
                  <div className="px-2.5 py-1.5 border-b border-slate-100">
                    <p className="text-xs font-bold text-slate-900">Archivos y Copias de Seguridad</p>
                    <p className="text-[10px] text-slate-500">Descarga contingencias o restaura datos</p>
                  </div>
                  ...
                  <div className="py-1 space-y-0.5">
                    {onExportNextDayAppointmentsTxt && (
                      <button
                        ...
                        className="w-full text-left px-2.5 py-2 rounded-xl text-xs hover:bg-amber-50/80 text-slate-800 flex items-start gap-2.5 transition-colors cursor-pointer"
                      >
                        <div className="p-1.5 bg-amber-100 text-amber-800 rounded-lg shrink-0 mt-0.5">
                          <FileText className="w-3.5 h-3.5" />
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 text-xs leading-tight">Turnos del Día Siguiente (.txt)</p>
                          <p className="text-[10.5px] text-slate-500 leading-tight">Ideal para contingencia por si se corta internet</p>
                        </div>
                      </button>
                    )}
                    ...
                    {onExportContactsTxt && (
                      <button
                        ...
                        className="w-full text-left px-2.5 py-2 rounded-xl text-xs hover:bg-emerald-50/80 text-slate-800 flex items-start gap-2.5 transition-colors cursor-pointer"
                      >
                        <div className="p-1.5 bg-emerald-100 text-emerald-800 rounded-lg shrink-0 mt-0.5">
                          <Users className="w-3.5 h-3.5" />
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 text-xs leading-tight">Directorio de Pacientes (.txt)</p>
                          <p className="text-[10.5px] text-slate-500 leading-tight">Listado ordenado con teléfonos y coberturas</p>
                        </div>
                      </button>
                    )}
                    ...
                    <button
                      ...
                      className="w-full text-left px-2.5 py-2 rounded-xl text-xs hover:bg-blue-50/80 text-slate-800 flex items-start gap-2.5 transition-colors cursor-pointer"
                    >
                      <div className="p-1.5 bg-blue-100 text-blue-800 rounded-lg shrink-0 mt-0.5">
                        <Database className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 text-xs leading-tight">Copia Completa de Respaldo (.txt)</p>
                        <p className="text-[10.5px] text-slate-500 leading-tight">Guarda todos los datos para restaurar en cualquier momento</p>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        <p className="font-bold text-slate-900 text-xs leading-tight">Importar Copia de Seguridad (.txt)</p>
                        <p className="text-[10.5px] text-slate-500 leading-tight">Carga copias de respaldo o listas en .txt</p>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className="w-full text-left px-2.5 py-2 rounded-xl text-xs hover:bg-rose-50 text-rose-700 flex items-start gap-2.5 transition-colors cursor-pointer"
                    >
                      <div className="p-1.5 bg-rose-100 text-rose-700 rounded-lg shrink-0 mt-0.5">
                        <Trash2 className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <p className="font-bold text-rose-900 text-xs leading-tight">Vaciar o Restablecer Agenda</p>
                        <p className="text-[10.5px] text-rose-600/80 leading-tight">Opciones de limpieza y reseteo</p>
                      </div>
                    </button>
                    ...
                    {onLogout && (
                      <button
                        ...
                        className="w-full text-left px-2.5 py-2 rounded-xl text-xs hover:bg-slate-100 text-slate-700 flex items-start gap-2.5 transition-colors cursor-pointer"
                      >
                        <div className="p-1.5 bg-slate-200 text-slate-800 rounded-lg shrink-0 mt-0.5">
                          <ShieldAlert className="w-3.5 h-3.5" />
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 text-xs leading-tight">Cerrar Sesión del Consultorio</p>
                          <p className="text-[10.5px] text-slate-500 leading-tight">Bloquear acceso a la agenda</p>
                        </div>
                      </button>
                    )}
                  </div>
              ...
              {/* Hidden file input for TXT import (backup or plain lists) */}
              <input
                ref={fileInputRef}
                type="file"
                accept=".txt"
        ...
        {/* Filter Controls Bar - only for Contacts/Pacientes tab */}
        {activeTab === 'contacts' && (
          <div className="pt-2 border-t border-white/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
